...
def calculateGCTA(path_,pop_,threads_):
    ...
    if pop_ == 'NAfr':
        ...
        cmd = ['/raid/genevol/users/lucas/gcta/gcta64', '--mbfile' ,path_ + '/chrs.txt','--keep',filesamp,'--make-grm','--out',path_+'/GCTA','--thread-num',threads_]
    elif pop_ == 'Geuvadis':
        filesamp='/raid/genevol/users/lucas/heritability/01.Pipeline/Samples/samples.filt'
        cmd = ['/raid/genevol/users/lucas/gcta/gcta64', '--mbfile' ,path_ + '/chrs.txt','--keep',filesamp,'--make-grm','--out',path_+'/GCTA','--thread-num',threads_]
    else:
        cmd = ['/raid/genevol/users/lucas/gcta/gcta64', '--mbfile' ,path_ + '/chrs.txt','--make-grm','--out',path_+'/GCTA','--thread-num',threads_]
